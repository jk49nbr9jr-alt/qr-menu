...
export default async function handler(req: Request) {
  ...
  const pwd = p[username];
  if (!pwd) return new Response(JSON.stringify({ ok:false, error:'no-pending' }), { status: 400 });

  if (!u.allowed.includes(username)) u.allowed.push(username);
  u.passwords[username] = pwd; // (Optional: hash)
  delete p[username];
