...
export default async function handler(req: Request) {
  ...
  if (username === 'admin') return new Response(JSON.stringify({ ok:false, error:'no-admin-delete' }), { status: 400 });

  const { users } = await ensureFiles(tenant);
  const u = await readJSON<{allowed:string[], passwords:Record<string,string>}>(users, {allowed:["admin"], passwords:{}});
  u.allowed = u.allowed.filter(x => x !== username);
  delete u.passwords[username];
  await writeJSON(users, u);
